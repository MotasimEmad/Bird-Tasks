@extends('layouts.app')
@section('content')
    <header class="flex items-center px-12 mt-6 justify-between">
            ...
            <div class="mb-6">
                ...
                @foreach ($project->tasks as $task)
                    <form method="POST" action="{{ route('tasks.update', $task->id) }}">
                        @method('PATCH')
                        @csrf
                        <div class="flex items-center bg-white shadow rounded-lg mb-3 p-5">
                            <label class="w-full {{ $task->completed ? 'text-gray-400' : ''}}">
                                <input class="focus:outline-none" name="body" value="{{ $task->body }}"></input>
                            </label>
